...
* 選択範囲を一時ファイルに書き出し、ポップアップサイズを見積もる（最大表示幅・行数。ANSI 除去・CJK は 2 桁換算）。まず幅を決め、その内側幅で折り返したときの実際の表示行数から高さを算出するため、横に長い行が折り返しても十分な高さを確保する。`@translate_popup_width`/`_height` を上限、読みやすい最小値を下限とし、選択範囲が少ないときは小さく表示する。
